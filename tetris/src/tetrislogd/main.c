...
// Exit gracefully
static void handleSigterm(int sig) // SIGTERM -> termination
{
    (void)sig;
    running = 0;
}
...
//
static void handleSighup(int sig) // SIGHUP -> signal hang up
{
    (void)sig;
...
/* ----- CONFIG ----- */
// Load configuration
static void loadConfig(const char *rc_path, char *log_path, size_t log_path_size,
...
int main(int argc, char *argv[])
{
    ...
    while (running)
    {
        ...
        if (activity > 0 && FD_ISSET(sock, &fds))
        {
            ...
            {
                LogRecord record;
                unpackLogRecord(buffer, &record);

                SenderTracker *t = findTracker(trackers, &tracker_count, record.pid);
                if (t != NULL)
                {
