...
void camera_init(Vector2 target_pos)
{
    screen_size = (Vector2){GetScreenWidth(), GetScreenHeight()};
    camera = (Camera2D){
        .target = Vector2Scale(target_pos, CONFIG.PHYSICS.TILE_SIZE),
...
void camera_update(Vector2 target_pos)
{
    camera.target = Vector2Scale(target_pos, CONFIG.PHYSICS.TILE_SIZE);
    ...
    if (map_max.x < screen_size.x)
        camera.offset.x = screen_size.x - (map_max.x - (float)screen_size.x * 0.5f);
    else if (map_min.x > 0.0f)
        camera.offset.x = (float)screen_size.x * 0.5f - map_min.x;
    if (map_max.y < screen_size.y)
        camera.offset.y = screen_size.y - (map_max.y - (float)screen_size.y * 0.5f);
    else if (map_min.y > 0.0f)
        camera.offset.y = (float)screen_size.y * 0.5f - map_min.y;
}
