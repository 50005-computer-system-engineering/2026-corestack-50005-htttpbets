...
void input_update(void) {
    ...
    if (IsKeyDown(CONFIG.KEYS.MOVE_UP) || IsKeyDown(CONFIG.KEYS.MOVE_DOWN) 
    || IsKeyDown(CONFIG.KEYS.MOVE_LEFT) || IsKeyDown(CONFIG.KEYS.MOVE_RIGHT)) {
        // Calculate movement direction, this allows players to move diagonally!
        MoveEventArgs args = {
            .player_id = 1,
            .x = IsKeyDown(CONFIG.KEYS.MOVE_RIGHT) ? 1
                : IsKeyDown(CONFIG.KEYS.MOVE_LEFT) ? -1 : 0,
            .y = IsKeyDown(CONFIG.KEYS.MOVE_UP) ? 1
                : IsKeyDown(CONFIG.KEYS.MOVE_DOWN) ? -1 : 0
        };
