...
void calc_player_spawns(int num_players, int inner_size, int spawns[][2]) {
    ...
    if (inner_size < 1) inner_size = 1; // Prevent division by 0
    ...
    printf("num_strips: %d\n", num_strips);
    if (num_strips < 1) num_strips = 1; // prevent division by 0
...
void map_generate(int num_players) {
    ...
    for (int i = 0; i < num_players; i++) {
        ...
        if (spawns[i][0] > 1) map[spawns[i][0] - 1][spawns[i][1]] = EMPTY; // Left
        if (spawns[i][0] < map_size - 2) map[spawns[i][0] + 1][spawns[i][1]] = EMPTY; // Right
        if (spawns[i][1] > 1) map[spawns[i][0]][spawns[i][1] - 1] = EMPTY; // Top
        if (spawns[i][1] < map_size - 2) map[spawns[i][0]][spawns[i][1] + 1] = EMPTY; // Bottom
    }
    free(spawns);
